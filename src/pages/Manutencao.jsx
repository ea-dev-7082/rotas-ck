...
import {
  Wrench, Plus, Filter, Download, Fuel, BarChart3, Loader2,
} from "lucide-react";
...
/**
 * Busca TODOS os registros de uma entidade paginando em batches.
 * Tenta .filter() com owner primeiro; se a entidade não tiver campo owner,
 * faz .list() genérico com filtro local.
 */
async function fetchAllByOwner(entity, ownerEmail, sortField = "-created_date") {
  let allData = [];
  let offset = 0;
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    try {
      ...
      // Fallback: .list() sem filtro server-side
      batch = await entity.list(sortField, API_BATCH_SIZE, offset);
    }
  ...
  // Garante ownership no client-side (safety net)
  return allData.filter(
    (item) => item.owner === ownerEmail || item.created_by === ownerEmail
  );
}
...
export default function Manutencao() {
  ...
  const loadVeiculos = useCallback(async () => {
    ...
    try {
      const data = await fetchAllByOwner(
        base44.entities.Veiculo,
        currentUser.email
      );
      setVeiculos(data);
    } catch (error) {
  ...
  // ========== CARREGAMENTO COMPLETO DE MANUTENÇÕES ==========
  const loadRegistros = useCallback(async () => {
    if (!currentUser?.email) return;
    setIsLoadingRegistros(true);
    try {
      const data = await fetchAllByOwner(
        base44.entities.ManutencaoVeiculo,
        currentUser.email,
        "-data"
      );
      setRegistros(data);
  ...
  }, [currentUser?.email, loadVeiculos, loadRegistros]);

  const isLoading = isLoadingVeiculos || isLoadingRegistros;

  // ========== INVALIDAÇÃO (recarrega tudo) ==========
  ...
  const filteredRegistros = useMemo(() => {
    return registros.filter((reg) => {
      if (filterVeiculo !== "todos" && reg.veiculo_id !== filterVeiculo)
        return false;
      if (filterTipo !== "todos" && reg.tipo !== filterTipo) return false;
      if (startDate && moment(reg.data).isBefore(moment(startDate)))
        return false;
      if (endDate && moment(reg.data).isAfter(moment(endDate).endOf("day")))
        return false;
      return true;
    });
  }, [registros, filterVeiculo, filterTipo, startDate, endDate]);

  // Relatório de custo (sem filtro por tipo)
  ...
  const handleExportCSV = useCallback(() => {
    ...
    const headers = [
      "Data", "Veículo", "Placa", "Tipo", "Km", "Valor",
      "Litros", "R$/L", "Local", "Descrição",
    ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-slate-100 p-6">
      <div className="container mx-auto max-w-6xl">
        ...
        {/* ALERTAS DE MANUTENÇÃO — usa TODOS os registros */}
