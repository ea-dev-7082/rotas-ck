...
export default function Optimizer() {
  ...
  // ========== CARREGAMENTO COMPLETO DE CLIENTES ==========
  const [allClientes, setAllClientes] = useState([]);
  const [isLoadingClientes, setIsLoadingClientes] = useState(false);
  const [clientesFullyLoaded, setClientesFullyLoaded] = useState(false);

  const loadAllClientes = useCallback(async () => {
    if (!currentUser || isLoadingClientes) return;
  ...
  const isLoading = isLoadingClientes && allClientes.length === 0;

  // ========== CARREGAMENTO COMPLETO DE VEÍCULOS ==========
  const [allVeiculos, setAllVeiculos] = useState([]);
  const [isLoadingVeiculos, setIsLoadingVeiculos] = useState(false);

  const loadAllVeiculos = useCallback(async () => {
    if (!currentUser || isLoadingVeiculos) return;
  ...
  // ========== CARREGAMENTO COMPLETO DE MOTORISTAS ==========
  const [allMotoristas, setAllMotoristas] = useState([]);
  const [isLoadingMotoristas, setIsLoadingMotoristas] = useState(false);

  const loadAllMotoristas = useCallback(async () => {
    if (!currentUser || isLoadingMotoristas) return;
  ...
  const mapboxToken =
    configs.find((c) => c.chave === "mapbox_token")?.valor || MAPBOX_TOKEN;
  const logoUrl = configs.find((c) => c.chave === "logo_url")?.valor || "";
  const nomeEmpresa =
    configs.find((c) => c.chave === "nome_empresa")?.valor || "";
  const tempoParadaEntrega =
    parseInt(
      configs.find((c) => c.chave === "tempo_parada_entrega")?.valor
    ) || 20;
  const margemTransito =
    parseInt(configs.find((c) => c.chave === "margem_transito")?.valor) || 10;
  ...
  const selectedVeiculoData = useMemo(
    () => veiculos.find((v) => v.id === selectedVeiculo),
  ...
  useEffect(() => {
    ...
    if (rotaAgendadaId && clientes.length > 0 && clientesFullyLoaded) {
      loadRotaAgendada(rotaAgendadaId);
    }
  }, [clientes, clientesFullyLoaded]);

  const loadRotaAgendada = async (rotaId) => {
    try {
      const rotasAgendadas = await base44.entities.RotaAgendada.filter({
        id: rotaId,
      });
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      if (clientesSemCoord.length > 0) {
        console.warn(
          `⚠️ ${clientesSemCoord.length} cliente(s) sem coordenadas (serão ignorados):`,
          clientesSemCoord.map((c) => c.nome).join(", ")
        );
      }

      const pontosParaOtimizar = [
        matrizGeocodificada,
        ...clientesFinal,
      ...
      if (pontosParaOtimizar.length < 2) {
        throw new Error(
          "Não há coordenadas suficientes para traçar a rota. Verifique os endereços."
        );
      }

      const optimizationData = await optimizeRoute(
        pontosParaOtimizar,
        mapboxToken
      );
...
CRITÉRIOS: Raio de 5-7 km do cliente mais distante OU mesmo bairro.`,
    ...
    try {
      ...
      newEntregas.forEach((item) => {
        if (item.client_name && item.latitude)
          coordsMap[item.client_name.trim()] = {
            lat: item.latitude,
            lng: item.longitude,
          };
      });
      ...
      const semCoords = remainingPart.filter(
        (p) => !p.latitude || !p.longitude
      );
      ...
      if (route) {
        const totalDrivingMinutes = legs.reduce(
          (sum, leg) =>
            sum +
            Math.round(((leg.duration || 0) / 60) * TRAFFIC_BUFFER),
          0
        );
  ...
  const buildManualRoute = (matrizGeocodificada, entregas, startTime) => {
    ...
    entregas.forEach((item, idx) => {
      const geocoded =
        geocodedClients.find(
          (g) => g.nome?.trim() === item.client_name?.trim()
        ) || item;

      if (idx > 0) currentTime += tempoParadaEntrega + 5;
      else currentTime += 10;
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">
                {editingRotaAgendadaId
                  ? "Editando Rota Agendada"
                  : "Otimizador de Rotas"}
              </h1>
                  ...
                  >
                    {isOptimizing ? (
                      ...
                      <>
                        ...
                        Otimizar ({selectedClients.length})
                      </>
                    )}
                  </Button>
                  {optimizedRoute && (
                ...
                <Card className="bg-white shadow-xl h-full min-h-[500px]">
                  <CardContent className="p-12 flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p className="text-gray-500 max-w-sm">
                      Selecione os clientes e clique em "Otimizar Rota".
                    </p>
                  </CardContent>
                </Card>
              )}
      ...
      <PrintModal
        ...
        onSaveAgendado={async (data) => {
          if (editingRotaAgendadaId) {
            await base44.entities.RotaAgendada.update(
              editingRotaAgendadaId,
              data
            );
            setEditingRotaAgendadaId(null);
            ...
            const result = await base44.entities.RotaAgendada.create({
              ...data,
              owner: currentUser?.email,
            });
            return result;
